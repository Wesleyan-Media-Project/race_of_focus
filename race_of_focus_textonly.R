...
# Output files
path_out_rdata <- "data/race_of_focus_textonly.rdata"
path_out_csv <- "data/race_of_focus_textonly.csv"

# Read full 1.18m file
df <- fread(path_full118m, encoding = "UTF-8") %>%
  select(ad_id, pd_id, page_name, region_distribution, detected_entities)
full_size <- nrow(df)
...
df$all_entities <- 
  ...
  str_split(",") %>%
  lapply(str_trim)
df <- df %>% select(-c(detected_entities))
